...
def revision(path_raw_data, new_file_name):
    ...
    # Missing the first few measures or the last few
    if len(new_df) != 1440:
...
def treating_data(filename):
    ...
    file_path = os.path.join(PATH_DATA_GIJON_1440, file_name)
...
def push_into_db(filename, datalogger_db):
    ...
    # Remove duplicate rows from the new data based on 'Datetime' column
    new_data.drop_duplicates(subset='Datetime', inplace=True)
...
def main(file_full_path):
    ...
    # Decide whether the file is complete or not
...
if __name__ == '__main__':
    ...
    PATH_DATA_GIJON_24 = r"C:\Users\pablo\Documents\Teleco2018-\5CURSO\TFG\TratarDatos\DatosTratados\DatosTratados_24_Gijon"  r""
